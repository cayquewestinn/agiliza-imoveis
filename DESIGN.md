...
- `Agenda.jsx`: the "today" calendar cell reuses the ring language (border + inset box-shadow) at the existing `--radius-sm` day-cell size, without the full `.carimbo` component, its `mix-blend-mode`, or the rotation — **do not add `mix-blend-mode: multiply` to `.agenda-calendar-cell-day`**, it was tried and made the day number disappear in dark mode (the day number is real text content, not a decorative overlay like the other two placements); **do not add the `~-11deg` rotation either**, it was tried and read as a crooked/broken square at the 24px day-cell size instead of a recognizable stamp (flagged by the user 2026-08-21) — the rotation only reads as intentional at the larger sizes the other two placements use.
