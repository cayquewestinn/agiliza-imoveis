...
- Theming lives in `src/index.css` as CSS custom properties: a `:root` block (light, default) plus a `:root[data-theme="dark"]` override. **Realigned 2026-08-25** to Able Development's design system (see `DESIGN.md`): navy `#13315C` is the one identity accent (stays a real, lightened color `#6F9BD6` in dark mode too — dark mode is a conventional dark theme, not monochrome or an inverted panel scheme). Status badges read from color + label only, no border-style-as-state and no rotation. Don't introduce a new color without reusing the existing `--status-*` / `--bg-*` / `--ink-*` / `--accent*` tokens.
